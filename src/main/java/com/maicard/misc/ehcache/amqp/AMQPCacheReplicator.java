...
public class AMQPCacheReplicator  implements CacheReplicator {
	    ...
	    private static final Logger LOG = Logger.getLogger(AMQPCacheReplicator.class.getName());
	    ...
	    private List<AsyncAMQPEventMessage> replicationQueue;
	     ...
	     */
	    public AMQPCacheReplicator(boolean replicatePuts, boolean replicateUpdates,
	    ...
	    private void replicatePut(Ehcache cache, Element element) {
	        AMQPEventMessage message = new AMQPEventMessage(Action.PUT, element.getKey(), element, cache.getName(), null);
	    ...
	    private void replicateRemoval(Ehcache cache, Element element) {
	        ...
	        AMQPEventMessage message = new AMQPEventMessage(Action.REMOVE, element.getKey(), null, cache.getName(),
	                null);
	    ...
	    public void notifyRemoveAll(Ehcache cache) {
	        ...
	        AMQPEventMessage message = new AMQPEventMessage(Action.REMOVE_ALL, null, null, cache.getName(), null);
	     ...
	     * @param message
	     */
	    protected void sendNotification(Ehcache cache, AMQPEventMessage message) {
	     ...
	     */
	    protected static final class AsyncAMQPEventMessage {
	        private Ehcache cache;
	        private AMQPEventMessage message;

	        /**
	         * @param cache   -
	         * @param message -
	         */
	        public AsyncAMQPEventMessage(Ehcache cache, AMQPEventMessage message) {
	            this.cache = cache;
	         ...
	         * @return the message
	         */
	        public AMQPEventMessage getMessage() {
	            return message;
	        }
	    }
	    ...
	    private void addMessageToQueue(Ehcache cache, AMQPEventMessage message) {
	        synchronized (replicationQueue) {
	            replicationQueue.add(new AsyncAMQPEventMessage(cache, message));
	        }
	    }
	    ...
	    private void flushReplicationQueue() {
	        ...
	        List<AsyncAMQPEventMessage> replicationQueueCopy;
	        ...
	        synchronized (replicationQueue) {
	            ...
	            //free up write queue
	            replicationQueueCopy = new ArrayList<AsyncAMQPEventMessage>(replicationQueue);
	        ...
	        for (AsyncAMQPEventMessage message : replicationQueueCopy) {
